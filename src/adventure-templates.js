...
function organizerNotes(world) {
  return [
    ...
    "SHOPS AND PROMISES: This new starter includes a small BAZAAR with a fictional resource and finite stock. Use Manage BAZAAR to review prices and grant a starting balance with a reason before play. Purchases need server confirmation. In Exchanges, players can offer selected items or resources alongside readings; both review and confirm the same offer, then all transfers complete together. Nothing is reserved while players negotiate. For the final scene, one player may propose an OATHBOOK agreement naming the duties, check-in time, participants, and an optional witness. Each participant accepts the exact written terms separately. A third player may witness when available. Optional fixed resource payments settle only after everyone confirms fulfillment, or an organizer explicitly adjudicates accepted terms. Narrative promises still need conversation and human judgment.",
    "COOPERATIVE PROPS: In Prepare SIGIL, review and publish the optional three-step challenge. Print its separate prop label. After examining evidence-core, a group can use one host player’s approved character and device to assign two in-person roles, review the host’s components, and follow the timed checkpoints. The starter requires the host’s Pocket notebook without consuming it. Authored consuming requirements are deducted only on successful completion; available quantities are checked again at the finish. Only the host receives the resulting journal and cooperation flag. A successful character cannot repeat that challenge for another outcome. Failed or cancelled attempts can start again. Other players can host their own attempt with their own supplies.",
    "SIGIL TIMERS AND READINGS: A host can pause, resume or cancel. Losing the device connection freezes the challenge when its server lease expires, within twenty seconds of the last confirmed heartbeat; returning requires an explicit resume. Event pause or disabling SIGIL pauses it immediately. Resuming the event does not resume individual challenges. Staff can operate a session with a recorded reason, including a manual success that still checks and consumes required supplies. Ending the event cancels unfinished challenges. In Prepare STATIC, review and publish the fictional zone reading, then print its label. The starter changes its prepared reading after the host completes SIGIL. Staff can select a published state with a reason and later restore the prepared behavior. STATIC values describe the story only. Collecting saves that reading in the character’s journal; current readings and timers always need a connection. Prop view offers a clear exit, optional sound, and visible cues. Keep privileged organizer accounts attended. Rehearsal copies keep authored challenges and readings with fresh prop codes; reset clears their attempts, outcomes, collected readings, and staff overrides while preserving the source event.",
    ...
    "REHEARSAL AND RECOVERY: Use Create rehearsal copy before a practice run. The dedicated copy has fresh gameplay progress and approved unassigned characters, so assign its characters again. Reset only that rehearsal copy while it is in Rehearsal; it is safe to repeat without wiping the source event's progress. Journal readings persist after refresh. Newly protected reveals and actions need connectivity; already permitted cached readings, when available, are read-only offline. The puzzle route itself consumes no inventory. Optional purchases, trades and accepted resource settlements change assets only when confirmed online; scene RSVPs do not fulfill promises. Rehearsal copies start with authored shop stock and zero balances. Reset clears rehearsal trades, balances and agreements, restores its captured starting inventory and initial shop stock, and keeps the original event unchanged.",
  ].join("\n\n");
}
...
export function buildAdventureTemplate(themeID) {
  ...
  const definition = {
    ...
    flags: [
      ...
      { id: "cooperation-complete", name: "Cooperative procedure completed" },
      { id: "cooperation-failed", name: "Cooperative procedure needs another attempt" },
    ],
  ...
  const characters = [world.roleOne, world.roleTwo].map((role, index) => ({ profile: validateCharacterProfile({
    ...
    startingEquipment: [
      ...
      { name: "Pocket notebook", quantity: 1, notes: "Record clues and the final agreement. The puzzle route consumes no equipment." },
    ],
  }, setup, []) }));
