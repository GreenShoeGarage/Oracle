...
const STATIC={
  '/app.js':['app-v13.js','text/javascript'],
...
export function createAppV13({ pool, config, logger }) {
  ...
  return async function handle(req,res){
    ...
    if(asset&&['GET','HEAD'].includes(req.method)){
      try{const file=await readFile(new URL(`../public/${asset[0]}`,import.meta.url));res.writeHead(200,{'Content-Type':`${asset[1]}; charset=utf-8`,'Cache-Control':'no-store','X-ORACLE-Shell-Version':'1.3.0','X-Content-Type-Options':'nosniff'});return res.end(req.method==='HEAD'?undefined:file);}catch{return delegate(req,res);}
    }
    ...
    try{if(await connections({req,res,path:url.pathname,url,method:req.method,user}))return;return delegate(req,res);}catch(error){const status=Number.isInteger(error.status)?error.status:500;if(status===500)logger?.({event:'connection_request_failed',code:error.code||'INTERNAL_ERROR'});res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});res.end(JSON.stringify({error:status===500?'Unable to complete the connection-card request.':error.message}));}
  };
}
