...
export function createKitUI(ctx) {
  ...
  function apply() { applyTheme(['detail', 'characters', 'adventure', 'adventure-manage', 'exchanges', 'sharing', 'story', 'trace'].includes(state.view) ? state.event?.setup?.theme : THEMES[0]); document.querySelector('[data-action="kit-collapse"]')?.setAttribute("aria-expanded", String(preferences.collapsed === "open")); }
  function controls() {
