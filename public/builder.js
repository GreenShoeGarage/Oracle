...
export function createKitUI(ctx) {
  ...
  function apply() { applyTheme(state.view === 'detail' ? state.event?.setup?.theme : THEMES[0]); document.querySelector('[data-action="kit-collapse"]')?.setAttribute("aria-expanded", String(preferences.collapsed === "open")); }
  function controls() {
  ...
  function rulesSummary(rules, theme) {
    ...
    return `<details class="panel rules-panel"><summary>Rules profile</summary><p class="hint">These are the event’s definitions. Character values and challenge resolution arrive with their gameplay tools.</p><div class="rule-columns"><section><h3>Attributes</h3>${list(rules.attributes, true)}</section><section><h3>${esc(theme.terms.expertise)}</h3>${list(rules.expertise, false)}</section><section><h3>${esc(theme.terms.resources)}</h3>${list(rules.resources, true)}</section><section><h3>Challenge outcomes</h3>${list(rules.outcomes, false)}</section></div></details>`;
  }
  function contentCards(setup, audience) {
    if (!setup.enabledInstruments.includes('briefing')) return '<section class="panel"><h2>Briefing is turned off</h2><p class="hint">Your organizer has not enabled any instruments yet.</p></section>';
  ...
  function enterEvent(ev) {
    if (eventId !== ev.id) { mode = ['owner', 'organizer'].includes(ev.role) ? 'organizer' : 'player'; eventId = ev.id; }
    if (!['owner', 'organizer'].includes(ev.role) && mode === 'organizer') mode = 'player';
    preview = null;
  }
