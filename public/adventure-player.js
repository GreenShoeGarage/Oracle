...
export function createAdventurePlayer(ctx) {
  ...
  function wayfinder(node) {
    const full = !node.operations?.canJoin && node.attendanceCount >= node.maxPlayers, closed = node.availability !== 'open', scheduled = node.availability === 'scheduled';
    return `<p class="prose">${esc(node.body)}</p><dl class="adv-scene-facts"><div><dt>Location</dt><dd>${esc(node.location || 'Ask your organizer')}</dd></div><div><dt>Play style</dt><dd>${esc(node.playStyle)}</dd></div><div><dt>Duration</dt><dd>${Number(node.durationMinutes)} minutes</dd></div><div><dt>Players</dt><dd>${Number(node.attendanceCount)} joined · ${Number(node.minPlayers)}–${Number(node.maxPlayers)} places</dd></div>${node.startsAt ? `<div><dt>Starts</dt><dd>${esc(formatDate(node.startsAt))}</dd></div>` : ''}${node.endsAt ? `<div><dt>Ends</dt><dd>${esc(formatDate(node.endsAt))}</dd></div>` : ''}</dl>${node.operations ? `<section class="adv-state-banner"><p>${esc(node.operations.reason || (node.operations.partyStatus === 'dispatched' ? 'Your party has been dispatched. You can join this scene.' : 'STAGEHAND coordinates admission to this scene.'))}</p>${node.operations.returnBy ? `<p>Return by ${esc(formatDate(node.operations.returnBy))}.</p>` : ''}${!preview ? `<button class="mt" data-action="stagehand-open" data-character="${esc(snapshot.character.id)}" data-encounter="${esc(node.operations.encounterId)}" ${!connected() ? 'disabled' : ''}>Open STAGEHAND</button>` : ''}</section>` : ''}${node.joined ? sceneLeave(node) : `<form id="adv-join-form" class="adv-action-form" data-kind="join" data-node="${esc(node.id)}">${err}<p class="hint">${node.operations && !node.operations.canJoin ? esc(node.operations.reason || 'Check your party assignment in STAGEHAND.') : scheduled ? 'This scene opens at its scheduled start time. Refresh then to join.' : closed ? 'This scene is closed.' : full ? 'This scene is full. Refresh later to check for a place.' : node.attendanceCount < node.minPlayers ? 'Gather the remaining players before starting the scene.' : 'There is room for your character.'}</p><button type="submit" class="primary" ${!canAct() || full || closed ? 'disabled' : ''}>Join scene</button></form>`}`;
  }
  function reading(entry) {
    return `<article class="adv-reading">${entry.type === 'whisper' ? '<p class="eyebrow">WHISPER · Collected account, not confirmed fact</p>' : entry.type === 'shared_reading' ? '<p class="eyebrow">Shared reading</p>' : entry.type === 'exchange_receipt' ? '<p class="eyebrow">Confirmed exchange</p>' : ''}<div class="adv-reading-heading"><h3>${esc(entry.title)}</h3><time datetime="${esc(entry.createdAt)}">${esc(formatDate(entry.createdAt))}</time></div><p class="prose">${esc(entry.text)}</p>${entry.audio ? `<audio controls preload="none" src="${esc(entry.audio)}" aria-label="Audio for ${esc(entry.title)}">Your browser does not support audio playback.</audio><p class="hint">The written reading remains available alongside the audio.</p>` : ''}</article>`;
  }
