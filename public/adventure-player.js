...
export function createAdventurePlayer(ctx) {
  ...
  function reading(entry) {
    return `<article class="adv-reading">${entry.type === 'shared_reading' ? '<p class="eyebrow">Shared reading</p>' : entry.type === 'exchange_receipt' ? '<p class="eyebrow">Confirmed exchange</p>' : ''}<div class="adv-reading-heading"><h3>${esc(entry.title)}</h3><time datetime="${esc(entry.createdAt)}">${esc(formatDate(entry.createdAt))}</time></div><p class="prose">${esc(entry.text)}</p>${entry.audio ? `<audio controls preload="none" src="${esc(entry.audio)}" aria-label="Audio for ${esc(entry.title)}">Your browser does not support audio playback.</audio><p class="hint">The written reading remains available alongside the audio.</p>` : ''}</article>`;
  }
