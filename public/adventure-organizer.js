...
export function createAdventureOrganizer(ctx) {
  ...
  async function action(button) {
    ...
    else if (name === 'advedit-rehearsal') { requireSaved(); confirmation('rehearsal', 'Create rehearsal copy', 'Create a separate rehearsal with fresh character identities, starting inventory, and authored initial shop stock. Balances start at zero. Discoveries, invitations, transactions, agreements, and attendance are not copied.'); }
    else if (name === 'advedit-reset') { requireSaved(); confirmation('reset', 'Reset rehearsal progress', 'Are you sure? This clears discoveries, attempts, journals, attendance, trades, balances, and agreements in this rehearsal. Inventory returns to the captured starting loadouts and shops return to initial stock. The adventure, characters, and authored shops remain.'); }
