import { createExperienceStudioUI } from './experience-studio-ui.js';
"use strict";
...
const experienceStudio = createExperienceStudioUI({ state, api, shell, esc, isManager, onReturnToEvent: loadEvent });
...
const install = createInstallUI({ getDirty: () => true, confirmDiscard: () => {
  ...
  return experienceStudio.confirmDiscard() && (field.canUpdate ? field.canUpdate() : true);
}, onChange: updateInstallUI, toast });
function resetPrivateViews() {
  experienceStudio.reset();
  commandDeck.reset();
  field.reset();
  fieldSync.reset();
  adventure.reset();
...
function render() {
  ...
  if (state.view === "experience-studio" && state.event) return experienceStudio.render();
...
async function loadEvents() {
  ...
  if (!location.hash && ["command-deck", "experience-studio"].includes(requested.get("view"))) {
    const targetView = requested.get("view");
    ...
    if (state.events.some(e => e.id === eventId)) { await loadEvent(eventId); if (targetView === "experience-studio") await experienceStudio.open(); else await commandDeck.open(); }
...
async function action(button) {
  ...
  if (["studio-open", "command-deck-open", "events", "account", "logout", "open-event", "create", "kit-import", "character-open", "adv-open", "advedit-open", "advedit-starter", "offline-open", "admin-open", "exchange-open", "sharing-open", "story-open", "story-manage", "trace-open", "bazaar-open", "bazaar-manage", "oath-open", "oath-manage", "sigil-open", "sigil-manage", "static-open", "static-manage", "stagehand-open", "stagehand-manage", "field-open"].includes(destination)) {
    ...
    if (state.view === "experience-studio" && !destination.startsWith("studio-")) experienceStudio.reset();
  }
  if (state.view === "experience-studio" && destination === "kit-view") {
    if (!experienceStudio.confirmDiscard()) return;
    experienceStudio.reset(); state.view="detail";
  }
  if (state.view === "command-deck" && destination === "kit-view") {
    commandDeck.reset();
    state.view = "detail";
  }
  if (await experienceStudio.action(button)) return;
...
document.addEventListener("input", e => { experienceStudio.input(e.target); });
document.addEventListener("change", async (e) => {
  if (await experienceStudio.change(e.target)) return;
...
window.addEventListener("hashchange", async () => {
  try {
    if (["#prop/", "#exchange/", "#badge/", "#sigil/", "#static/"].some((prefix) => location.hash.startsWith(prefix)) && (!experienceStudio.confirmDiscard() || !field.confirmDiscard() || !kit.confirmDiscard() || !characters.confirmDiscard() || !adventure.confirmDiscard() || !adventureOrganizer.confirmDiscard() || !sharing.confirmDiscard() || !exchanges.confirmDiscard() || !story.confirmDiscard() || !trace.confirmDiscard() || !economy.confirmDiscard() || !oaths.confirmDiscard() || !sigil.confirmDiscard() || !signals.confirmDiscard() || !stagehand.confirmDiscard())) {
...
window.addEventListener("focus", () => { if (!state.busy) { void commandDeck.refresh(); void experienceStudio.refresh(); } });
...
async function checkConnection() {
  ...
  try {
    ...
    await experienceStudio.refresh();
