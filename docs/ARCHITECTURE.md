...
Application v1.2.0 · Database schema 10 · Briefing-pack format 1 · Adventure format 1

Current release `705a3e702a42f4148ea12a74fd1e9eb93aef03ae` is deployed to staging and production as v1.2.0. Exact main and accepted staging verification, the complete remote all-twelve/all-three-theme journey with cleanup, Railway production, and exact readiness plus 104 public GET checks passed. Evidence, the unchanged-candidate staging retry, and physical-device/backup limits are recorded in [STATUS.md](STATUS.md).
...
**Public entry and startup.** `public/index.html` supplies a static app description, features, theme examples, help/catalog links, and canonical public metadata. `public/landing.css` styles that signed-out introduction; authenticated/private views hide it. `public/display.js` applies the saved Dark/Outdoor presentation early, and browser color follows the actual display rather than an unrelated operating-system preference. `public/startup.js` owns a persistent polite status outside the replaced app, completes `aria-busy`, and focuses main content only if the user has not already interacted and no dialog has taken focus. The public homepage, help, and screenshot tour are the intentional sitemap entries; private APIs and event data remain outside public indexing and service-worker caching.

**Public screenshot tour.** `public/tour.html` explains existing features without registration, with 19 distinct screenshots and links to 21 complete examples in `public/tour-examples.html`. `scripts/tour/` supplies fictional in-memory fixtures to the existing UI renderers; the committed gallery strips executable scripts, forms, app controllers, and active game links. `public/tour-catalog.json` records renderer provenance. The development-only `jsdom` dependency is used for generation, not public serving. Screenshots are real browser captures of those detached examples, never live account/event screenshots. The server exposes an explicit public path/image allowlist; visiting these pages performs no private API reads or gameplay writes. Tour assets remain outside the field kit's service-worker cache. The tour is indexable on production; the auxiliary example gallery is `noindex,follow` and staging remains excluded. See [TOUR.md](TOUR.md) for regeneration and capture instructions.
