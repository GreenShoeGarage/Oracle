...
GitHub source control and Railway application/PostgreSQL hosting. Recorded September 6, 2026. Batch 7 (v0.7.0, schema 8) is an implemented candidate with full local verification and workflow evidence; exact-candidate CI, remote staging, and production are pending. Current production remains the verified Batch 6 v0.6.0/schema 7 release.

## Current infrastructure

Source: [GreenShoeGarage/Oracle](https://github.com/GreenShoeGarage/Oracle). Release commit: `9c14293bd6a13d5d5912070bdacda6b24308df43` (v0.6.0, schema version 7).

Staging deployment `edf175c1-e090-47ab-b6a2-62813b3e20cd` succeeded at this commit, and [staging CI 34004234793](https://github.com/GreenShoeGarage/Oracle/actions/runs/34004234793) passed the complete remote workflow. Production deployment `04ebac6c-854e-4cab-b0be-2bb00f5ec689` succeeded on the same commit. [Production CI 34004393610](https://github.com/GreenShoeGarage/Oracle/actions/runs/34004393610) passed exact-commit v0.6.0/schema 7 readiness and all 37 public GET checks without creating data. Exact run links and evidence are in [STATUS.md](STATUS.md).

| Target | Railway project | Project ID | Application URL | Status |
| --- | --- | --- | --- | --- |
| Staging | ORACLE Staging | `4e75d5ec-f8e9-492d-ae75-e0d44428d24a` | [Staging app](https://oracle-production-488d.up.railway.app) | v0.6.0/schema 7 deployed; complete story/TRACE/three-theme workflow passed |
| Production | ORACLE | `d1989864-9a40-4176-a9d3-203a06c4bd72` | [Production app](https://oracle.greenshoegarage.com) | v0.6.0/schema 7 live; exact-commit production smoke passed |
...
## Batch 7 candidate gates

The candidate local HTTP rehearsal passed 305 reads/342 writes, including previous three-theme adventures/exchanges/story and new finite-stock purchases, mixed asset barter, independent agreement witnessing, exact terms revisions, atomic settlement/retry, dispute/adjudication, linked correction, and real rehearsal reset. The 37-test migration/foundation set passed across the initial run and corrected-fixture rerun, with one existing TCP-only skip. All 13 new tables passed populated recovery-fixture constraints and repeat migration locally. Root integration checks passed 22 tests; kit/offline checks passed 27. Eight full-app DOM/API checks passed with zero uncaught errors and ten OATHBOOK UI scenarios passed. Economy HTTP passed seven tests, OATHBOOK thirteen, story thirteen, and exchange fourteen plus one TCP-only skip. Full local `npm run verify` passed 183 tests (179 passed, zero failures, four deliberate TCP-only skips), including syntax/version/static assets. Exact-candidate PostgreSQL contention/dump-restore/Docker CI, remote staging, production, and public checks are still pending. No candidate deployment ID or release SHA is recorded yet.

The deployed Batch 6 infrastructure record above remains the baseline until those gates pass. Migration 008 must preserve populated schema-7 story and investigation state, every older record, and the existing enabled operator's full identity/credential row. CI now restores populated fictional resources, finite shops, transactions/receipt grants, initial inventory baselines, trade snapshots, and exact agreement signatures/history in addition to all prior tables. The remote workflow adds a disposable independent witness account. Production checks remain read-only.
...
Use `main` for integration and `staging` for the staging application. The workflow listens to `main`, `staging`, and `production` pushes and pull requests. Its `verify` job runs on integration/staging commits and pull requests. Batch 7 retains all earlier gates and adds authoritative resource/stock ownership, atomic bilateral trades, exact agreement consent/witnessing/settlement, disputed outcomes and audited corrections, and economy-aware rehearsal reset. Migration rehearsals begin with populated schema-7 story/TRACE records, schema-6 exchanges, prior adventures/characters, and an existing enabled superuser. Recovery fixtures retain those records and populate every new economy/agreement table. On `staging`, a dependent `staging-smoke` job waits for the checked `GITHUB_SHA` to appear in readiness with the expected application and schema versions, then runs the authenticated two-account journey. Production promotion skips duplicate verification and runs `production-smoke`: up to five minutes waiting for that exact SHA, version, and schema at the canonical custom domain, followed by public GET checks. This relies on promoting the already-verified commit; the production job is not a replacement for CI and staging gates. Railway production follows the dedicated `production` release branch, currently at checked release commit `9c14293bd6a13d5d5912070bdacda6b24308df43`. Advance that branch only after checking CI and staging for the exact candidate commit. This is a manual promotion procedure: Railway's Wait for CI was unavailable through the connector and `checkSuites` remains false. No automatic branch protection is claimed.
...
Current production state (Batch 6): the reserved operator account is enabled. Migration and startup logs confirmed the matching existing account at this release; provisioning preserved its identity and password. No replacement user was created. The conditional instructions below remain the supported process for new installations or absent reserved accounts.
...
Migration 008 adds `economy_resources`, `economy_balances`, `economy_shops`, `economy_stock`, `economy_transactions`, `economy_receipts`, `economy_requests`, `economy_baselines`, `exchange_trade_offers`, `oath_agreements`, `oath_participants`, `oath_history`, and `oath_requests`. Migrations 001–007 remain unchanged. Existing accounts/credentials, event setup, characters/inventories, adventures/journals, policies/exchanges, story publications and collections, investigations, and all audit history are retained. Existing events receive no new resources, balances, shops, or agreements during migration. Migration checksums and transactions remain enforced; no destructive down migration is provided.

**After schema 8 is applied, the schema-7 v0.6.0 application is not a compatible rollback image.** Earlier releases are also incompatible. Do not deploy an older binary against this database, bypass readiness, or drop economy/agreement tables to force startup. Preserve data and roll forward with a tested schema-8-compatible fix. Subsequent app-only rollbacks require verified compatibility with the current schema and stored formats. A database restore remains a separate incident decision.
