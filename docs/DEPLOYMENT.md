...
GitHub source control and Railway application/PostgreSQL hosting. Recorded September 5, 2026. Batch 3 source targets application v0.3.0 and schema 4. Candidate verification is in progress; the last verified production release remains Batch 2 (v0.2.0, schema 2). Configure each service before its first source-backed deployment.

## Current infrastructure

Source: [GreenShoeGarage/Oracle](https://github.com/GreenShoeGarage/Oracle). Last verified production release commit: `537c17443947694664499d91007c289bdfc2ac6d` (v0.2.0, schema version 2).
...
4. Pass the two-account staging journey: event isolation, invitation redemption, roles, player/prop secret filtering, all three theme changes without record loss, organizer pack import as a separate Draft, player export filtering, stale-version rejection, persistence after a fresh sign-in, and immediate access removal. Batch 3 adds two-character approval, public-field and badge privacy, code rotation, inventory initialization/reapproval, prewritten assignment, malformed-write rejection, copies into another event, and revoked character/badge access. Cleanup archives disposable events and logs out sessions. This is HTTP workflow verification; it does not replace browser/device or human field testing.
5. Review the migration and recovery position before production schema changes. Migrations 003 and 004 add administration and character storage while preserving existing accounts, credentials, events, setup, memberships, invitations, and history. No live database backup or scheduled backup is currently available through the configured platform; record this unresolved limitation. Event-pack exports do not protect account or membership data. Prefer a verified protected database checkpoint when backup capability is available, and use the schema-4 roll-forward procedure below.
...
Migrations 003 and 004 add default-false account administration flags, system audit history, event character settings, factions, characters, and separate inventory. Earlier setup migration 002 remains unchanged. Existing event and account records, credentials, memberships, invitations, and history are retained. Migration checksums and transactions remain enforced; there are no destructive down migrations.

**After schema 4 is applied, neither v0.1.0 nor v0.2.0 is a compatible rollback image.** Those binaries require older exact schemas and cannot serve the upgraded database. Do not deploy them against schema 4, disable readiness checks, or drop the new tables/columns to force startup. Preserve data and roll forward with a tested schema-4-compatible fix. Validate migration preservation, character approval/inventory, and role enforcement against staging before promotion.

For subsequent schema-4 releases, application-only rollback is appropriate only after verifying that the target image supports current schema and stored formats. A database restore is never an automatic companion to application rollback.
