...
Real iPhone/Android airplane-mode cold starts, installation, cameras, assistive technology, and the human field pilot remain unrun. The [pilot checklist](PILOT.md) is the record for those observations; automated HTTP/storage/DOM checks cannot substitute for them. Sustained Railway capacity and scheduled live database backups also remain unverified/unconfigured. The application rollback helper is not integrated into the recovery runner or CI and has no accepted rehearsal result.

## Previous verified release — ORACLE v1.0.0 / Batch 12

Release `782671590b3ea3547f0bd3a24a7a98ed5af76dd4` shipped public Help & guides, organizer/player documentation, the install panel layout correction, and live-event reset regression coverage. It retained schema 10 and existing stored formats/contracts.

| Check | Recorded result |
| --- | --- |
| [Main CI 34041397663](https://github.com/GreenShoeGarage/Oracle/actions/runs/34041397663) | Passed the exact candidate, including PostgreSQL tests, isolated load, populated recovery, startup, and Docker gates |
| [Staging CI 34041498209](https://github.com/GreenShoeGarage/Oracle/actions/runs/34041498209) | Passed the same candidate, including the complete all-twelve-instrument/all-three-theme remote journey |
| PostgreSQL suite | 309 tests: 308 passed, zero failures, one PGlite-only skip |
| Railway production | Deployment `bc9ea961-3b1d-4201-b68e-c45d73c03bca` succeeded on the exact release at 15:19:37 UTC on September 6 |
| [Production CI 34041923997](https://github.com/GreenShoeGarage/Oracle/actions/runs/34041923997) | Exact v1.0.0/schema-10/commit readiness and 72 public GET paths passed, without gameplay writes |

Production startup retained the existing enabled project superuser. Automated install-module tests passed; the layout correction was not visually verified on a physical device. `scripts/rollback-rehearsal.js` was added but was not wired into CI or run, so application rollback is not a completed Batch 12 gate. The human/device pilot remained outstanding.

---

## Previous verified release — ORACLE v0.11.0 / Batch 11

Recorded September 6, 2026.

Source: [GreenShoeGarage/Oracle](https://github.com/GreenShoeGarage/Oracle). Application `0.11.0`; database schema `10`; briefing/adventure formats `1`; browser journal archive version `2`. Batch 11's usability and pilot-preparation implementation is complete and deployed at [oracle.greenshoegarage.com](https://oracle.greenshoegarage.com), release `6127db477f76f1c3b9f18ea6eb423e029c9dcaf7`. Both exact-commit PostgreSQL CI runs, separate 100-player isolated measurements, Railway staging, the complete remote all-twelve/all-three-theme workflow, and Railway production passed. Exact production readiness plus 70 public GET paths passed with no verification writes. The existing enabled superuser retained identity/password. Human/device checks remain unrun and require actual participants and devices.
...
- A bounded 100-authenticated-player isolated PostgreSQL load rehearsal with measured latency/concurrency, integrity checks, and disposable cleanup. The actual 100-player short burst passed both exact-commit CI runs separately; it is separate from the small lifecycle test and establishes only the recorded isolated workload.
- No SQL, pack, archive-format, or gameplay API-contract change. Existing operator identity/password and persisted event data remain preserved.
...
| Load tooling | Four focused tests passed, including a two-player lifecycle and bounded non-forced cleanup; the separate 100-player measurement passed corrected exact main CI |
...
| Exact-release main PostgreSQL CI | [main CI 34038628533](https://github.com/GreenShoeGarage/Oracle/actions/runs/34038628533), [verification job 101501202501](https://github.com/GreenShoeGarage/Oracle/actions/runs/34038628533/job/101501202501), passed commit `6127db477f76f1c3b9f18ea6eb423e029c9dcaf7`: 307 tests, 306 passed, zero failures, one PGlite-only skip; all twelve TCP gates, populated schema-10 recovery, repeated migration/startup, and production Docker secure-cookie/graceful-stop checks passed |
| Main CI isolated 100-player measurement | 1,800 HTTP 200 responses, zero errors/timeouts; 100 player connections, 4.772 seconds, p50 184.12 ms / p95 462.87 ms; integrity and cleanup passed; short CI burst only |
| Staging CI isolated 100-player measurement | 1,800 HTTP 200 responses, zero errors/timeouts; 100 player connections, 4.176 seconds, p50 137.4 ms / p95 501.4 ms; integrity and cleanup passed; separate short CI burst, not Railway load |
| Exact-release staging PostgreSQL CI | [staging CI 34038793396](https://github.com/GreenShoeGarage/Oracle/actions/runs/34038793396), [verification job 101501643902](https://github.com/GreenShoeGarage/Oracle/actions/runs/34038793396/job/101501643902), passed the same commit: 307 tests, 306 passed, zero failures, one PGlite-only skip; all twelve TCP, load, recovery, startup, and production-image gates passed |
| Railway staging | Deployment `1a67d055-f042-4e52-a457-bbeeb6ba29f1` succeeded on the exact candidate at 14:19:45 UTC; schema 10 / v0.11.0 staging startup confirmed |
| Complete remote workflow | [Remote workflow job 101501900225](https://github.com/GreenShoeGarage/Oracle/actions/runs/34038793396/job/101501900225) passed exact readiness, 70 public GET paths, all twelve connected instruments in Fantasy/Cyberpunk/Wasteland, starter/field/revocation workflows, instrument/STAGEHAND copied play/reset, and cleanup at 14:28:40 UTC |
| Public desktop browser | v0.11.0 loaded; skip/main focus target, visible 44-pixel buttons, no horizontal overflow, zero ORACLE console errors; no authenticated/mobile/physical-device acceptance inferred |
| Railway production and public smoke | Deployment `42f6212a-27ad-4847-bc79-a08796bd162a` succeeded on the exact release; [production CI 34039312308](https://github.com/GreenShoeGarage/Oracle/actions/runs/34039312308), [public smoke job 101503054655](https://github.com/GreenShoeGarage/Oracle/actions/runs/34039312308/job/101503054655), passed exact readiness and 70 public GET routes with zero verification writes |
...
- The 100-player isolated short-burst measurement passed. Sustained event load, the actual Railway configuration, and real event-network behavior remain unmeasured; no production-capacity claim follows from this CI run.
...
- At the Batch 11 handoff, human/device acceptance and the Batch 12 release handoff remained open. Later releases are recorded above; automated deployment evidence does not close human acceptance or establish production capacity.
