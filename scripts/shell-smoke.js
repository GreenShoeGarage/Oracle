...
import { COMMAND_DECK_SHELL_ASSETS } from '../src/app-v19.js';
...
const expectedWorker = await readFile('public/sw-v19.js', 'utf8');
const match = /const STATIC_ASSETS=\[([\s\S]*?)\];/.exec(expectedWorker);
assert.ok(match, 'Cannot find the current offline asset manifest.');
const paths = [...new Set([...match[1].matchAll(/'([^']+)'/g)].map(entry => entry[1]).concat(Object.keys(COMMAND_DECK_SHELL_ASSETS)))];
let totalBytes = 0;
for (const path of paths) {
  ...
  if (Object.hasOwn(COMMAND_DECK_SHELL_ASSETS, path)) {
    assert.deepEqual(bytes, await readFile(`public/${COMMAND_DECK_SHELL_ASSETS[path][0]}`), `${path}: deployed bytes must match the checked commit`);
  }
