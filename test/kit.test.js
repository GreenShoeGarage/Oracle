...
test("built-in themes, templates and unavailable instruments have coherent data", () => {
  ...
  assert.equal(INSTRUMENTS.filter((instrument) => !instrument.available).length, 12);
  assert.deepEqual(INSTRUMENTS.filter((instrument) => instrument.available).map((instrument) => instrument.id), ["briefing"]);
...
test("rules are bounded declarative records and cannot smuggle formulas or capabilities", () => {
  const mutations = [
    ...
    (setup) => { setup.enabledInstruments = ["relic"]; },
