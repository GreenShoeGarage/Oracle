...
import { createAppV23, EXPERIENCE_STUDIO_SHELL_ASSETS } from '../src/app-v23.js';
...
test('every offline shell asset has the current release identity through the production handler', async (t) => {
  ...
  const server = createServer(createAppV23({ pool, config }));
  ...
  const worker = await readFile('public/sw-v23.js', 'utf8');
  ...
  for (const path of new Set([...paths, ...Object.keys(EXPERIENCE_STUDIO_SHELL_ASSETS)])) {
    ...
    if (Object.hasOwn(EXPERIENCE_STUDIO_SHELL_ASSETS, path)) {
      const [file, mime] = EXPERIENCE_STUDIO_SHELL_ASSETS[path];
